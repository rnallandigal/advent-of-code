...
using sections_t = std::
    unordered_map<std::string, std::pair<std::string, std::vector<range_t>>>;
...
input_t get_input(std::string const& in) {
    ...
    for(auto section : aoc::split(in, "\n\n")) {
        ...
        destination = lines[0].substr(
            hyphen_idx + 4, lines[0].find(' ') - hyphen_idx - 4
        );
...
merge_events(std::vector<std::vector<int64_t>> const& mappings) {
    ...
    for(int i = 0; i < (int)events.size(); i++) {
        if(merged_events.empty()) merged_events.push_back(events[i]);
...
std::vector<std::vector<int64_t>> iterate(
