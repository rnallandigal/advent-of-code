...
std::array<std::function<std::pair<int, int>(int, int, int)>, T> transforms{
    // transform    -> border
...
std::vector<vertex_t> arrange_tiles(jigsaw_t const& jigsaw) {
    ...
    for(int i = 1; i < N; i++) {
        ...
        auto [prev_target_t, curr_target_t] =
            i % D ? std::pair(RIGHT, LEFT) : std::pair(BOTTOM, TOP);

        int prev_border_t = compose[prev_target_t][inv[prev_t]];
        auto [curr_id, curr_border_t] =
            edges[std::pair(prev_id, prev_border_t)];
...
tile_t
assemble_image(jigsaw_t const& jigsaw, std::vector<vertex_t> const& tilemap) {
    int N = (int)jigsaw.size(), D = (int)std::sqrt(N),
...
int count_roughness(tile_t& image, tile_t const& monster) {
    int N = (int)image.size(), X = (int)monster[0].size(),
        Y = (int)monster.size();
