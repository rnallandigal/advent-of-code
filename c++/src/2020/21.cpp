...
std::string part1(std::string const & in) {
	...
	for(auto [ingredients, _] : clues) {
		for(auto ingredient : ingredients) soln += !unsafes.count(ingredient);
	}
...
std::string part2(std::string const & in) {
	...
	auto serialize = [](items_vec const & items, std::string const & item) {
		return std::lower_bound(items.begin(), items.end(), item) - items.begin();
	};
	...
	items_vec soln; soln.reserve(n);
	for(int i = 0; i < n; i++) soln.push_back(all_ingredients[matching[i]]);
