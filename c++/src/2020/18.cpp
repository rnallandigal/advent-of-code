...
int64_t shunting_yard(std::string const & expr, int add, int mul) {
	...
	std::vector<char> shunt(100, '\0');	int depth = -1;
	auto prec = [add, mul](char c) { return c == '+' ? add : mul; };
	auto apply_op = [&](char op) {
		int64_t b = output.top(); output.pop();
		int64_t a = output.top(); output.pop();
	...
	for(auto c : expr) {
		if(c == ' ') continue;
			...
			while(shunt[depth] != '(') apply_op(shunt[depth--]);
			if(depth >= 0) depth--;
		} else if(c == '+' || c == '*') {
			while(
				depth >= 0 &&
	...
	while(depth >= 0) apply_op(shunt[depth--]);
	return output.top();
}
